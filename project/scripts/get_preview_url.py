...
def get_tracks_from_artistid(artistid):
    ...
    try:
        ...
        if len(releases) == 0:
            return None
        releases_ids = list(map(lambda x: int(x.getAttribute('id')), releases))
        res = {}
...
def get_url(h5path, oauth_client_key):
    # sanity checks
    ...
    if track_7digitalid >= 0:
        ...
        if preview == '':
            #print('something went wrong when looking by track id for ' + str(track_7digitalid))
            pass
        else:
            print(preview)
            sys.exit(0)
    ...
    if release_7digitalid >= 0:
        ...
        if tracks_name_ids is None:
            #print('something went wrong when looking by album id for ' + str(release_7digitalid))
            pass
        else:
            ...
            if preview == '':
                #print('something went wrong when looking by track id after release id for ' + str(track_name))
                pass
            else:
                print(preview)
                sys.exit(0)
    ...
    if artist_7digitalid >= 0:
        ...
        if tracks_name_ids is None:
            #print('something went wrong when looking by artist id ' + str(artist_7digitalid))
            pass
        else:
            closest_track = get_closest_track(tracks_name_ids.keys(),track_name)
            if closest_track != track_name:
            ...
            preview = get_preview_from_trackid(tracks_name_ids[closest_track])
            if preview == '':
                #print('something went wrong when looking by track id after artist id ' + str(closest_track))
                pass
            else:
                print(preview)
                sys.exit(0)
